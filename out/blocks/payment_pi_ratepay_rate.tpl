    ...
    <dd class="[{if $oView->getCheckedPaymentId() == $paymentmethod->oxpayments__oxid->value}]activePayment[{/if}]" [{if $oView->getCheckedPaymentId() != $paymentmethod->oxpayments__oxid->value}]style="display:none"[{/if}]>
        ...
        <div class="form" style="margin-top: 15px">
            ...
            <p style="margin-bottom: 15px;">
                ...
                <input name='pi_ratepay_rate_fon' type='text' value='' size='37' class='form-control'/>
            </p>
            [{/if}]
            [{if isset($pi_ratepay_rate_birthdate_check)}]
            <p style="margin-bottom: 15px;">
                <label>[{oxmultilang ident="PI_RATEPAY_RATE_VIEW_PAYMENT_BIRTHDATE"}]</label>
                <input name='pi_ratepay_rate_birthdate_day' maxlength='2' type='text' value='' data-fieldsize='small' class='form-control'/>
                <input name='pi_ratepay_rate_birthdate_month' maxlength='2' type='text' value='' data-fieldsize='small' class='form-control'/>
                <input name='pi_ratepay_rate_birthdate_year' maxlength='4' type='text' value='' data-fieldsize='small' class='form-control'/>
                <span class='note'>[{oxmultilang ident="PI_RATEPAY_RATE_VIEW_PAYMENT_BIRTHDATE_FORMAT"}]</span>
            </p>
            [{/if}]
            [{if isset($pi_ratepay_rate_company_check)}]
            <p style="margin-bottom: 15px;">
                <label>[{oxmultilang ident="PI_RATEPAY_RATE_VIEW_PAYMENT_COMPANY"}]</label>
                <input name='pi_ratepay_rate_company' maxlength='255' size='37' type='text' value='' class='form-control'/>
            </p>
            [{/if}]
            [{if isset($pi_ratepay_rate_ust_check)}]
            <p style="margin-bottom: 15px;">
                <label>[{oxmultilang ident="PI_RATEPAY_RATE_VIEW_PAYMENT_UST"}]</label>
                <input name='pi_ratepay_rate_ust' maxlength='255' size='37' type='text' value='' class='form-control'/>
            </p>
            [{/if}]
        ...
        [{if $pi_ratepay_rate_activateelv == 1}]
        <div id="pi_ratepay_rate_bank_box" class="form">
            <p>
                <label>[{oxmultilang ident="PI_RATEPAY_ELV_VIEW_BANK_OWNER"}]:</label>
                <input name='pi_ratepay_rate_bank_owner' maxlength='255' size='37' type='text' value='[{$piDbBankowner}]' class='form-control'/>
            </p>
            <p>
                <label>[{oxmultilang ident="PI_RATEPAY_ELV_VIEW_BANK_ACCOUNT_NUMBER"}]:</label>
                <input name='pi_ratepay_rate_bank_account_number' maxlength='255' size='37' type='text' value='[{$piDbBankaccountnumber}]' class='form-control'/>
            </p>
        </div>
        [{/if}]
        [{ if isset($error)}]
