...
abstract class EncryptionAbstract
{
    ...
    {
        $aSelectSql = $this->_createBankdataSelectSql($sUserId);
        $aBankdata = $this->_selectBankdataFromDatabase($aSelectSql);
    ...
    public function saveBankdata($sUserId, array $aBankdata)
    {
        if ($this->isBankdataSetForUser($sUserId)) {
            $aSaveSql = $this->_createBankdataUpdateSql($sUserId, $aBankdata);
        } else {
            $aSaveSql = $this->_createBankdataInsertSql($sUserId, $aBankdata);
        }
        $this->_insertBankdataToDatabase($aSaveSql);
    }

    private function _createBankdataInsertSql($sUserId, array $aBankdata)
    {
        $aValues = [
            'userid' => ':userid'
        ];
    ...
    private function _createBankdataUpdateSql($sUserId, array $aBankdata)
    {
        $sKey = $this->_privateKey;
        $aSet = [];
    ...
    private function _createBankdataSelectSql($sUserId)
    {
    ...
    abstract protected function _selectBankdataFromDatabase($aSelectSql);
